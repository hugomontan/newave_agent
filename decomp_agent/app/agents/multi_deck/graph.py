...
from decomp_agent.app.agents.multi_deck.nodes.rag_nodes import (
    rag_simple_node,
    rag_enhanced_node,
)
from decomp_agent.app.agents.multi_deck.nodes.llm_nodes import (
    llm_planner_node,
)
...
NODE_DESCRIPTIONS = {
    ...
    "comparison_coder": {
        "name": "Code Generator", 
        "icon": "[CODE]",
        "description": "Gerando codigo Python para comparar os decks DECOMP..."
    },
    "comparison_executor": {
        "name": "Code Executor",
        "icon": "[EXEC]",
...
def create_multi_deck_agent() -> StateGraph:
    ...
    from decomp_agent.app.agents.multi_deck.nodes import (
    ...
    try:
        for event in agent.stream(initial_state, stream_mode="updates", config=config):
            for node_name, node_output in event.items():
                ...
                # Detalhes específicos de cada node (simplificado por enquanto)
