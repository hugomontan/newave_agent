...
class RestricoesVazaoHQTool(DECOMPTool):
    ...
    def _create_codigo_nome_mapping(self, dadger: Any, data_usinas: list) -> Dict[int, str]:
        ...
        # Tentar múltiplos caminhos possíveis (igual UHUsinasHidrelétricasTool)
        ...
        # Buscar apenas no primeiro deck NEWAVE encontrado
        for newave_decks_path in possible_newave_paths:
            if not os.path.exists(newave_decks_path):
                continue
            
            safe_print(f"[HQ] Buscando HIDR.DAT em: {newave_decks_path}")
                    ...
                    if os.path.exists(hidr_path):
                        try:
                            ...
                            if hidr.cadastro is not None and not hidr.cadastro.empty:
                                ...
                                safe_print(f"[HQ] ✅ Cache criado com {len(cache_completo)} usinas do deck {deck_dir}")
                                break
