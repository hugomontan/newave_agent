...
from decomp_agent.app.rag.vectorstore import get_embeddings
from decomp_agent.app.config import QUERY_EXPANSION_ENABLED, SEMANTIC_MATCH_MIN_SCORE, safe_print
...
def expand_query(query: str) -> str:
    ...
    Adaptado para termos específicos do DECOMP.
...
def _get_tool_embedding(tool: DECOMPTool, embeddings_model) -> Tuple[list[float], np.ndarray]:
    """
...
def _get_tool_embeddings_parallel(tools: list[DECOMPTool], embeddings_model, max_workers: int = 5) -> Dict[str, Tuple[list[float], np.ndarray]]:
    """
    ...
    if tools_to_process:
        ...
        def process_tool(tool: DECOMPTool) -> Tuple[str, list[float], np.ndarray]:
...
def find_best_tool_semantic(
    ...
    try:
        ...
        # Regra: Se score >= SEMANTIC_MATCH_MIN_SCORE, sempre executar a tool com maior score
        # Se score < SEMANTIC_MATCH_MIN_SCORE, nenhuma tool é executada (fluxo normal assume)
        safe_print("[SEMANTIC MATCHER] " + "=" * 70)
