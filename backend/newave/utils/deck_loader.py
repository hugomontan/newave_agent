...
from backend.newave.config import NEWAVE_DECKS_DIR, NEWAVE_DATA_DIR
...
def _find_decks_dir() -> Path:
    ...
    # 2. Fallback: procurar diretamente em data/newave (sem subpasta decks)
    if NEWAVE_DATA_DIR.exists() and NEWAVE_DATA_DIR.is_dir():
        # Verificar se há arquivos .zip diretamente em data/newave
        zip_files = list(NEWAVE_DATA_DIR.glob("NW*.zip"))
        if zip_files:
            return NEWAVE_DATA_DIR
    
    # 3. Fallback: procurar em locais antigos
    fallback_locations = [
        ROOT_DIR / "newave_agent" / "decks",
        ROOT_DIR / "decks",
...
def list_available_decks() -> List[DeckInfo]:
    ...
    # Buscar todos os arquivos .zip que seguem o padrão NW{YYYY}{MM} diretamente no diretório
    zip_files_direct = list(decks_dir.glob("NW*.zip"))
    for zip_file in zip_files_direct:
    ...
    # Se não encontrou nada diretamente, tentar busca recursiva limitada (apenas 1 nível)
    if not decks and decks_dir.exists():
        for subdir in decks_dir.iterdir():
            if subdir.is_dir():
                zip_files_in_subdir = list(subdir.glob("NW*.zip"))
                for zip_file in zip_files_in_subdir:
