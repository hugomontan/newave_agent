...
from backend.newave.config import NEWAVE_DOCS_DIR as DOCS_DIR, RAG_CHUNK_SIZE, RAG_CHUNK_OVERLAP
from backend.newave.rag.vectorstore import get_vectorstore, add_documents
...
def get_abstract_path() -> Path:
    ...
    from backend.newave.config import NEWAVE_DOCS_DIR
    
    # Tenta primeiro relativo ao NEWAVE_DOCS_DIR
    abstract_path = NEWAVE_DOCS_DIR / "abstract.md"
    ...
    raise FileNotFoundError(f"abstract.md não encontrado em {NEWAVE_DOCS_DIR}")
...
def get_specific_doc_path(file_name: str) -> Path:
    ...
    # Tenta primeiro relativo ao NEWAVE_DOCS_DIR
    specific_path = NEWAVE_DOCS_DIR / "specific" / doc_filename
