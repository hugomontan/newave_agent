...
from backend.decomp.config import DECOMP_DOCS_DIR as DOCS_DIR, RAG_CHUNK_SIZE, RAG_CHUNK_OVERLAP
from backend.decomp.rag.vectorstore import get_vectorstore, add_documents
...
def get_abstract_path() -> Path:
    ...
    from backend.decomp.config import DECOMP_DOCS_DIR
    
    # Tenta primeiro relativo ao DECOMP_DOCS_DIR
    abstract_path = DECOMP_DOCS_DIR / "abstract.md"
    ...
    raise FileNotFoundError(f"abstract.md não encontrado em {DECOMP_DOCS_DIR}")
...
def get_specific_doc_path(file_name: str) -> Path:
    ...
    # Tenta primeiro relativo ao DECOMP_DOCS_DIR
    specific_path = DECOMP_DOCS_DIR / "specific" / doc_filename
