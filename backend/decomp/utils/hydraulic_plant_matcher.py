"""
Serviço unificado de matching de usinas hidrelétricas para DECOMP.
...
tanto por nome oficial (nome_decomp) quanto por nome decomposto (nome_completo).
"""
import os
import re
import csv
...
DEFAULT_CSV_PATH = os.path.join(
    ...
    "data",
    "deparahidro_decomp.csv"
)


class DecompHydraulicPlantMatcher:
    """
    Serviço unificado de matching de usinas hidrelétricas para DECOMP.
    
    Utiliza um CSV de de-para para expandir abreviações dos nomes das usinas
    e fazer matching tanto por nome oficial (nome_decomp) quanto por
    nome decomposto (nome_completo curado).
    
    Attributes:
        abbrev_to_full: Mapeamento nome_decomp.lower() -> nome_completo.lower() (curado)
        full_to_abbrev: Mapeamento nome_completo.lower() -> nome_decomp.lower()
        code_to_names: Mapeamento codigo -> (nome_decomp, nome_completo, None)
    """
    ...
    def __init__(self, csv_path: str = None):
        ...
        self.code_to_names: Dict[int, Tuple[str, str, Optional[int]]] = {}  # codigo -> (nome_decomp, nome_completo, None)
    ...
    def _load_name_mapping_from_csv(self) -> None:
        ...
        - code_to_names: codigo -> (nome_decomp, nome_completo, None)
        """
        if not os.path.exists(self.csv_path):
            debug_print(f"[DECOMP_HYDRAULIC_MATCHER] ⚠️ CSV de de-para não encontrado: {self.csv_path}")
            debug_print("[DECOMP_HYDRAULIC_MATCHER] ⚠️ Matching funcionará sem expansão de abreviações")
            return
            ...
            with open(self.csv_path, 'r', encoding='utf-8') as f:
                ...
                for row in reader:
                    try:
                        ...
                        nome_decomp = str(row.get('nome_decomp', '')).strip()
                        nome_completo = str(row.get('nome_completo', '')).strip()
                        
                        if not nome_decomp:
                            continue
                        
                        # Se não tiver nome_completo, usar nome_decomp
                        ...
                        # Sempre criar mapeamento por código (sem posto em DECOMP)
                        self.code_to_names[codigo] = (nome_decomp, nome_completo, None)
                        
                    except (ValueError, KeyError) as e:
                        debug_print(f"[DECOMP_HYDRAULIC_MATCHER] ⚠️ Erro ao processar linha do CSV: {e}")
                        continue
            
            debug_print(f"[DECOMP_HYDRAULIC_MATCHER] ✅ CSV carregado: {len(self.abbrev_to_full)} mapeamentos de abreviações")
            debug_print(f"[DECOMP_HYDRAULIC_MATCHER] ✅ Total de usinas mapeadas: {len(self.code_to_names)}")
            
        except Exception as e:
            debug_print(f"[DECOMP_HYDRAULIC_MATCHER] ❌ Erro ao carregar CSV: {e}")
        ...
        for abbrev in sorted_abbrevs:
            ...
            if abbrev in expanded:
                ...
                if re.search(pattern, expanded):
                    ...
                    debug_print(f"[DECOMP_HYDRAULIC_MATCHER] ✅ Abreviação expandida: '{abbrev}' -> '{full_name}'")
        
        if expanded != query_lower:
            debug_print(f"[DECOMP_HYDRAULIC_MATCHER] Query expandida: '{query_lower}' -> '{expanded}'")
    ...
    def _create_expanded_list_from_csv(self) -> Tuple[List[str], Dict[str, int]]:
        ...
        O CSV é a fonte de verdade. Cria lista incluindo tanto nome_decomp
        quanto nome_completo de cada entrada do CSV.
        
        Returns:
            Tupla (available_names, name_to_codigo) onde:
            - available_names: Lista de nomes (nome_decomp + nome_completo)
            - name_to_codigo: Mapeamento nome.lower() -> codigo (do CSV)
        """
        available_names = []
        name_to_codigo = {}  # Mapeamento nome -> codigo (do CSV)
        
        safe_print(f"[DECOMP_HYDRAULIC_MATCHER] Criando lista expandida de nomes do CSV...")
        for codigo, (nome_decomp, nome_completo, _) in self.code_to_names.items():
            # Adicionar nome_decomp (nome do deck)
            if nome_decomp and nome_decomp.strip():
                available_names.append(nome_decomp)
                name_to_codigo[nome_decomp.lower()] = codigo
                safe_print(f"[DECOMP_HYDRAULIC_MATCHER]   Código {codigo}: nome_decomp='{nome_decomp}'")
            
            # Adicionar nome_completo (se diferente do nome_decomp)
            if nome_completo and nome_completo.strip():
                nome_completo_lower = nome_completo.lower()
                nome_decomp_lower = nome_decomp.lower() if nome_decomp else ''
                if nome_completo_lower != nome_decomp_lower:
                    available_names.append(nome_completo)
                    name_to_codigo[nome_completo_lower] = codigo
                    safe_print(f"[DECOMP_HYDRAULIC_MATCHER]   Código {codigo}: nome_completo='{nome_completo}'")
        
        safe_print(f"[DECOMP_HYDRAULIC_MATCHER] Lista expandida criada: {len(available_names)} nomes únicos do CSV")
    ...
    def _find_plant_in_dataframe_by_name(
        ...
        """
        Busca planta no DataFrame pelo nome (nome_decomp do CSV).
        ...
        (idx, posto, etc.) usando o nome_decomp do CSV.
        
        Args:
            nome_decomp: Nome da usina como aparece no deck (do CSV)
            plants_list: Lista de plantas do DataFrame
            
        Returns:
            Dict da planta encontrada ou None
        """
        nome_decomp_upper = nome_decomp.upper().strip()
        for plant in plants_list:
            nome_usina = str(plant.get('nome', '')).upper().strip()
            if nome_usina == nome_decomp_upper:
                return plant
        return None
    ...
    def _prepare_plants_list(
        ...
        [{'codigo': int, 'nome': str, 'idx': Optional[int]}, ...]
            ...
            if match:
                try:
                    ...
                    if codigo in valid_codes:
                        debug_print(f"[DECOMP_HYDRAULIC_MATCHER] ✅ Código {codigo} encontrado por padrão numérico")
                        return codigo
                except ValueError:
                    continue
    ...
    def _extract_by_name(
        ...
        Cria lista expandida incluindo tanto nome_decomp quanto nome_completo do CSV.
        ...
        if not available_names:
            safe_print(f"[DECOMP_HYDRAULIC_MATCHER] ⚠️ Nenhum nome disponível no CSV para matching")
            return None
        
        # ETAPA 1: Match exato (tanto nome_decomp quanto nome_completo do CSV)
        debug_print(f"[DECOMP_HYDRAULIC_MATCHER] ETAPA 1: Tentando match exato e por word boundaries...")
        debug_print(f"[DECOMP_HYDRAULIC_MATCHER]   Query (lowercase): '{query_lower}'")
        # Ordenar por tamanho (maior primeiro) para priorizar matches mais específicos
        csv_entries = []
        for codigo, (nome_decomp, nome_completo, _) in self.code_to_names.items():
            csv_entries.append({
                'codigo': codigo,
                'nome_decomp': nome_decomp,
                'nome_completo': nome_completo
            })
        sorted_entries = sorted(csv_entries, key=lambda x: len(x['nome_decomp']), reverse=True)
        
        for entry in sorted_entries:
            codigo = entry['codigo']
            nome_decomp = entry['nome_decomp']
            nome_completo = entry['nome_completo']
            
            # Match exato contra nome_decomp do CSV
            if nome_decomp and nome_decomp.lower().strip() == query_lower.strip():
                debug_print(f"[DECOMP_HYDRAULIC_MATCHER] ✅ Código {codigo} encontrado por match exato (nome_decomp CSV): '{nome_decomp}'")
                return codigo
            
            # Match exato contra nome_completo do CSV
            if nome_completo and nome_completo.lower().strip() == query_lower.strip():
                debug_print(f"[DECOMP_HYDRAULIC_MATCHER] ✅ Código {codigo} encontrado por match exato (nome_completo CSV): '{nome_completo}'")
                return codigo
            ...
            if nome_decomp and len(nome_decomp.lower().strip()) >= 4:
                nome_decomp_lower = nome_decomp.lower().strip()
                # Verificar se todas as palavras do nome estão na query
                palavras_nome = nome_decomp_lower.split()
                if len(palavras_nome) == 1:
                    # Nome de uma palavra: usar word boundary
                    pattern = r'\b' + re.escape(nome_decomp_lower) + r'\b'
                    if re.search(pattern, query_lower):
                        debug_print(f"[DECOMP_HYDRAULIC_MATCHER] ✅ Código {codigo} encontrado por nome_decomp '{nome_decomp}' na query (word boundary)")
                        return codigo
                else:
                    ...
                    if todas_palavras_presentes:
                        debug_print(f"[DECOMP_HYDRAULIC_MATCHER] ✅ Código {codigo} encontrado por nome_decomp '{nome_decomp}' na query (todas palavras presentes)")
                        return codigo
            ...
            if nome_completo and len(nome_completo.lower().strip()) >= 4:
                ...
                if len(palavras_nome) == 1:
                    ...
                    if re.search(pattern, query_lower):
                        debug_print(f"[DECOMP_HYDRAULIC_MATCHER] ✅ Código {codigo} encontrado por nome_completo '{nome_completo}' na query (word boundary)")
                        return codigo
                else:
                    # Nome com múltiplas palavras: verificar se todas estão presentes
                    todas_palavras_presentes = all(
                    ...
                    if todas_palavras_presentes:
                        debug_print(f"[DECOMP_HYDRAULIC_MATCHER] ✅ Código {codigo} encontrado por nome_completo '{nome_completo}' na query (todas palavras presentes)")
                        return codigo
        
        # ETAPA 2: Fuzzy matching contra lista expandida do CSV
        debug_print(f"[DECOMP_HYDRAULIC_MATCHER] ETAPA 2: Tentando fuzzy matching com matcher centralizado...")
        ...
        if match_result:
            ...
            if matched_lower in name_to_codigo:
                ...
                debug_print(f"[DECOMP_HYDRAULIC_MATCHER] ✅ Código {codigo} encontrado via matcher centralizado: '{matched_name}' (score: {score:.2f})")
                return codigo
            else:
                debug_print(f"[DECOMP_HYDRAULIC_MATCHER] ⚠️ Nome '{matched_name}' encontrado mas não está no mapeamento name_to_codigo")
        else:
            debug_print(f"[DECOMP_HYDRAULIC_MATCHER] ⚠️ Matcher centralizado não encontrou match acima do threshold {threshold}")
        
        # ETAPA 3: Fallback com palavras-chave (usando CSV)
        debug_print(f"[DECOMP_HYDRAULIC_MATCHER] ETAPA 3: Tentando fallback keyword matching no CSV...")
    ...
    def _fallback_keyword_matching_csv(self, query_lower: str) -> Optional[int]:
        ...
        Busca tanto em nome_decomp quanto em nome_completo do CSV.
        ...
        safe_print(f"[DECOMP_HYDRAULIC_MATCHER] Palavras significativas da query: {palavras_query}")
        
        if not palavras_query:
            safe_print(f"[DECOMP_HYDRAULIC_MATCHER] Nenhuma palavra significativa encontrada na query")
            return None
        ...
        for codigo, (nome_decomp, nome_completo, _) in self.code_to_names.items():
            # Testar tanto nome_decomp quanto nome_completo
            for nome in [nome_decomp, nome_completo]:
                if not nome:
                    continue
        ...
        if candidatos:
            ...
            safe_print(f"[DECOMP_HYDRAULIC_MATCHER] Top 5 candidatos (fallback CSV):")
            for idx, (cod, nome, score) in enumerate(candidatos[:5], 1):
                safe_print(f"[DECOMP_HYDRAULIC_MATCHER]   {idx}. Código {cod}: '{nome}' (score: {score:.2f})")
            
            melhor_codigo, melhor_nome, melhor_score = candidatos[0]
            safe_print(f"[DECOMP_HYDRAULIC_MATCHER] ✅ Código {melhor_codigo} encontrado (fallback CSV): '{melhor_nome}' (score: {melhor_score:.2f})")
            return melhor_codigo
        
        safe_print(f"[DECOMP_HYDRAULIC_MATCHER] Nenhum candidato encontrado no fallback")
        return None
    ...
    def extract_plant_from_query(
        ...
        return_format: str = "codigo",  # "codigo" | "tupla"
        threshold: float = 0.5
        ...
        Args:
            ...
            return_format: Formato de retorno desejado ("codigo", "tupla")
            threshold: Score mínimo para fuzzy matching (0.0 a 1.0)
            
        Returns:
            Código da usina, tupla (código, idx_real) ou None
        """
        if not query:
            return None
        ...
        if not plants_list:
            debug_print("[DECOMP_HYDRAULIC_MATCHER] ⚠️ Nenhuma planta disponível para matching")
            return None
        ...
        safe_print(f"[DECOMP_HYDRAULIC_MATCHER] ===== INÍCIO: Matching de Usina Hidrelétrica =====")
        safe_print(f"[DECOMP_HYDRAULIC_MATCHER] Query original: '{query}'")
        safe_print(f"[DECOMP_HYDRAULIC_MATCHER] Formato de retorno: {return_format}")
        safe_print(f"[DECOMP_HYDRAULIC_MATCHER] Threshold: {threshold}")
        safe_print(f"[DECOMP_HYDRAULIC_MATCHER] Plantas no DataFrame: {len(plants_list)}")
        safe_print(f"[DECOMP_HYDRAULIC_MATCHER] Códigos no CSV: {len(valid_codes_csv)}")
        safe_print(f"[DECOMP_HYDRAULIC_MATCHER] Códigos no DataFrame: {len(valid_codes_df)}")
        
        # ETAPA 2: Expandir abreviações
        expanded_query = self._expand_abbreviations(query)
        if expanded_query != query.lower():
            safe_print(f"[DECOMP_HYDRAULIC_MATCHER] Query expandida: '{query.lower()}' -> '{expanded_query}'")
        else:
            safe_print(f"[DECOMP_HYDRAULIC_MATCHER] Query não foi expandida (sem abreviações detectadas)")
        
        # ETAPA 3: Tentar extrair código numérico (validar contra CSV)
        debug_print(f"[DECOMP_HYDRAULIC_MATCHER] ETAPA 3: Tentando extrair código numérico...")
        codigo = self._extract_numeric_code(expanded_query, valid_codes_csv)
        if codigo is None and expanded_query != query.lower():
            debug_print(f"[DECOMP_HYDRAULIC_MATCHER] Tentando extrair código da query original...")
            codigo = self._extract_numeric_code(query, valid_codes_csv)
        
        if codigo is not None:
            debug_print(f"[DECOMP_HYDRAULIC_MATCHER] ✅ Código {codigo} encontrado por padrão numérico (CSV)")
            # Obter nome_decomp do CSV para buscar no DataFrame
            if codigo in self.code_to_names:
                nome_decomp_csv, _, _ = self.code_to_names[codigo]
                debug_print(f"[DECOMP_HYDRAULIC_MATCHER]   Buscando no DataFrame pelo nome: '{nome_decomp_csv}'")
                # Buscar no DataFrame usando NOME do CSV (não código!)
                plant = self._find_plant_in_dataframe_by_name(nome_decomp_csv, plants_list)
                if plant:
                    result = self._format_return(plant, return_format)
                    debug_print(f"[DECOMP_HYDRAULIC_MATCHER] ===== FIM: Matching concluído (código numérico) =====")
                    return result
                else:
                    debug_print(f"[DECOMP_HYDRAULIC_MATCHER] ⚠️ Código {codigo} (nome '{nome_decomp_csv}') encontrado no CSV mas não no DataFrame")
            else:
                debug_print(f"[DECOMP_HYDRAULIC_MATCHER] ⚠️ Código {codigo} não encontrado no code_to_names")
        else:
            debug_print(f"[DECOMP_HYDRAULIC_MATCHER] Nenhum código numérico encontrado")
        
        # ETAPA 4: Tentar match por nome (usando CSV diretamente)
        debug_print(f"[DECOMP_HYDRAULIC_MATCHER] ETAPA 4: Tentando match por nome no CSV...")
        codigo = self._extract_by_name(expanded_query, threshold)
        if codigo is None and expanded_query != query.lower():
            debug_print(f"[DECOMP_HYDRAULIC_MATCHER] Tentando match por nome na query original...")
            codigo = self._extract_by_name(query.lower(), threshold)
        
        if codigo is not None:
            safe_print(f"[DECOMP_HYDRAULIC_MATCHER] ✅ Código {codigo} encontrado no CSV pelo matching")
            # Buscar informações do CSV para debug
            if codigo in self.code_to_names:
                nome_decomp_csv, nome_completo_csv, _ = self.code_to_names[codigo]
                safe_print(f"[DECOMP_HYDRAULIC_MATCHER]   CSV: código={codigo}, nome_decomp='{nome_decomp_csv}', nome_completo='{nome_completo_csv}'")
                
                # Buscar no DataFrame usando NOME do CSV (não código!)
                debug_print(f"[DECOMP_HYDRAULIC_MATCHER]   Buscando no DataFrame pelo nome: '{nome_decomp_csv}'")
                plant = self._find_plant_in_dataframe_by_name(nome_decomp_csv, plants_list)
                if plant:
                    safe_print(f"[DECOMP_HYDRAULIC_MATCHER]   DataFrame: código={plant.get('codigo')}, nome='{plant.get('nome')}', idx={plant.get('idx')}")
                    result = self._format_return(plant, return_format)
                    safe_print(f"[DECOMP_HYDRAULIC_MATCHER] ===== FIM: Matching concluído (match por nome) - Retornando código {codigo} =====")
                    return result
                else:
                    debug_print(f"[DECOMP_HYDRAULIC_MATCHER] ⚠️ Código {codigo} (nome '{nome_decomp_csv}') encontrado no CSV mas não no DataFrame")
                    # Fallback: retornar baseado no formato mesmo sem encontrar no DataFrame
                    if return_format == "codigo":
                        safe_print(f"[DECOMP_HYDRAULIC_MATCHER] ===== FIM: Matching concluído (código do CSV) - Retornando código {codigo} =====")
                        return codigo
                    elif return_format == "tupla":
                        # Para tupla, usar código como idx_real quando não encontra no DataFrame
                        debug_print(f"[DECOMP_HYDRAULIC_MATCHER] ⚠️ Usando código {codigo} como idx_real (fallback)")
                        safe_print(f"[DECOMP_HYDRAULIC_MATCHER] ===== FIM: Matching concluído (código do CSV) - Retornando tupla ({codigo}, {codigo}) =====")
                        return (codigo, codigo)
            else:
                debug_print(f"[DECOMP_HYDRAULIC_MATCHER] ⚠️ Código {codigo} não encontrado no code_to_names")
        
        safe_print(f"[DECOMP_HYDRAULIC_MATCHER] ⚠️ Nenhuma usina específica detectada na query")
        safe_print(f"[DECOMP_HYDRAULIC_MATCHER]   Query testada: '{query}'")
        safe_print(f"[DECOMP_HYDRAULIC_MATCHER]   Query expandida testada: '{expanded_query}'")
        safe_print(f"[DECOMP_HYDRAULIC_MATCHER] ===== FIM: Matching concluído (sem resultado) =====")
        return None
    ...
    def _format_return(
        ...
        if return_format == "codigo":
            ...
            if idx_real is None:
                ...
                debug_print(f"[DECOMP_HYDRAULIC_MATCHER] ⚠️ idx_real não disponível, usando código {codigo}")
                idx_real = codigo
            return (codigo, idx_real)
        
        else:
            debug_print(f"[DECOMP_HYDRAULIC_MATCHER] ⚠️ Formato de retorno desconhecido: {return_format}, retornando código")
            return codigo


# Instância global para uso conveniente
_default_matcher: Optional[DecompHydraulicPlantMatcher] = None


def get_decomp_hydraulic_plant_matcher() -> DecompHydraulicPlantMatcher:
    """
    Obtém a instância global do DecompHydraulicPlantMatcher.
    
    Cria uma nova instância na primeira chamada (lazy initialization).
    
    Returns:
        Instância do DecompHydraulicPlantMatcher
    """
    global _default_matcher
    if _default_matcher is None:
        _default_matcher = DecompHydraulicPlantMatcher()
