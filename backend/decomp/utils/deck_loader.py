...
from backend.decomp.config import DECOMP_DECKS_DIR, DECOMP_DATA_DIR
from backend.core.config import ROOT_DIR

def _find_decks_dir() -> Path:
    """
    Encontra o diretório de decks, procurando em vários locais possíveis.
    
    Returns:
        Path do diretório de decks encontrado
    """
    # 1. Local principal (data/decomp/decks)
    if DECOMP_DECKS_DIR.exists():
        # Verificar se há arquivos .zip diretamente no diretório
        zip_files = list(DECOMP_DECKS_DIR.glob("*.zip"))
        if zip_files:
            return DECOMP_DECKS_DIR
        
        # Verificar se há pastas DC* diretamente no diretório (estrutura com subpastas)
        dc_dirs = [d for d in DECOMP_DECKS_DIR.iterdir() if d.is_dir() and d.name.startswith("DC")]
        if dc_dirs:
            return DECOMP_DECKS_DIR
        
        # Se não encontrou arquivos diretamente, verificar estrutura aninhada (decks/decks/)
        nested_decks_dir = DECOMP_DECKS_DIR / "decks"
        if nested_decks_dir.exists() and nested_decks_dir.is_dir():
            nested_zip_files = list(nested_decks_dir.glob("*.zip"))
            nested_dc_dirs = [d for d in nested_decks_dir.iterdir() if d.is_dir() and d.name.startswith("DC")]
            if nested_zip_files or nested_dc_dirs:
                return nested_decks_dir
        
        # Retornar o diretório principal mesmo sem arquivos (será usado para criar)
        return DECOMP_DECKS_DIR
    
    # 2. Fallback: procurar diretamente em data/decomp (sem subpasta decks)
    if DECOMP_DATA_DIR.exists() and DECOMP_DATA_DIR.is_dir():
        # Verificar se há arquivos .zip diretamente em data/decomp
        zip_files = list(DECOMP_DATA_DIR.glob("*.zip"))
        if zip_files:
            return DECOMP_DATA_DIR
    
    # 3. Fallback: procurar em locais alternativos
    fallback_locations = [
        ROOT_DIR / "decks",
    ...
    # 4. Retornar o diretório principal mesmo que não exista (será criado)
    return DECOMP_DECKS_DIR
...
def list_available_decks() -> List[DeckInfo]:
    ...
    # Recalcular o diretório dinamicamente para garantir que está correto
    decks_dir = _find_decks_dir()
    ...
    for month_dir in decks_dir.iterdir():
        ...
        print(f"[DEBUG DECOMP] list_available_decks: Buscando decks semanais em {month_name}")
        for sem_num in range(1, 6):
            ...
            if zip_file.exists():
                ...
                if parsed is None:
                    print(f"[DEBUG DECOMP] list_available_decks: Nome de deck inválido: {deck_name}")
                    continue
                ...
                print(f"[DEBUG DECOMP] list_available_decks: Deck adicionado: {deck_name} de {zip_file}")
...
def load_deck(deck_name: str) -> Path:
    ...
    # Recalcular o diretório dinamicamente
    decks_dir = _find_decks_dir()
