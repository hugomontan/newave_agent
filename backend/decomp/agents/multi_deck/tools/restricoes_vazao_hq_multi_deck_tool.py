"""
Tool Multi-Deck para consultar restrições de vazão (HQ) em múltiplos decks DECOMP.
Executa RestricoesVazaoHQTool ou RestricoesVazaoHQConjuntaTool em paralelo em todos os decks selecionados.
"""
...
from backend.decomp.tools.restricoes_vazao_hq_conjunta_tool import RestricoesVazaoHQConjuntaTool
...
class RestricoesVazaoHQMultiDeckTool(DECOMPTool):
    """
    Tool para consultar restrições de vazão (HQ/LQ/CQ) em múltiplos decks DECOMP.
    ...
    def __init__(self, deck_paths: Dict[str, str]):
        """
        Inicializa a tool multi-deck de restrições de vazão.
    ...
    def get_name(self) -> str:
        return "RestricoesVazaoHQMultiDeckTool"
    
    def can_handle(self, query: str) -> bool:
        """
        Verifica se a query é sobre restrições de vazão.
    ...
    def execute(self, query: str, **kwargs) -> Dict[str, Any]:
        """
        Executa a consulta de restrições de vazão em paralelo em todos os decks.
        ...
        # Determinar qual tool usar (conjunta ou unitária)
        query_lower = query.lower()
        is_conjunta = any(termo in query_lower for termo in [
            "conjunta", "somatorio", "somatório"
        ])
        
        tool_class = RestricoesVazaoHQConjuntaTool if is_conjunta else RestricoesVazaoHQTool
        tool_name_single = "RestricoesVazaoHQConjuntaTool" if is_conjunta else "RestricoesVazaoHQTool"
        
        safe_print(f"[RESTRICOES VAZAO HQ MULTI-DECK] ========== INÍCIO ==========")
        safe_print(f"[RESTRICOES VAZAO HQ MULTI-DECK] Query: {query[:100]}")
        safe_print(f"[RESTRICOES VAZAO HQ MULTI-DECK] Decks: {list(self.deck_paths.keys())}")
        safe_print(f"[RESTRICOES VAZAO HQ MULTI-DECK] Tool: {tool_name_single}")
        safe_print(f"[RESTRICOES VAZAO HQ MULTI-DECK] Workers: {self.max_workers}")
        
        # Executar consultas em paralelo
        safe_print(f"[RESTRICOES VAZAO HQ MULTI-DECK] Executando consultas em {len(self.deck_paths)} decks em paralelo...")
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    if result.get("success"):
                        ...
                        safe_print(f"[RESTRICOES VAZAO HQ MULTI-DECK] ✅ {deck_name}: {data_count} registro(s)")
                    else:
                        safe_print(f"[RESTRICOES VAZAO HQ MULTI-DECK] ❌ {deck_name}: {result.get('error', 'Erro desconhecido')}")
                        
                except FutureTimeoutError:
                    safe_print(f"[RESTRICOES VAZAO HQ MULTI-DECK] ⏱️ Timeout ao processar {deck_name}")
                    deck_results.append({
                        "name": deck_name,
                        "display_name": self.deck_display_names.get(deck_name, deck_name),
                        "result": {},
                    ...
                    safe_print(f"[RESTRICOES VAZAO HQ MULTI-DECK] ❌ Erro ao processar {deck_name}: {e}")
                    deck_results.append({
                        "name": deck_name,
                        "display_name": self.deck_display_names.get(deck_name, deck_name),
                        "result": {},
        ...
        safe_print(f"[RESTRICOES VAZAO HQ MULTI-DECK] ✅ {len(successful_results)}/{len(deck_results)} decks processados com sucesso")
        safe_print(f"[RESTRICOES VAZAO HQ MULTI-DECK] ========== FIM ==========")
    ...
    def _execute_single_deck(
        ...
        """Executa RestricoesVazaoHQTool ou RestricoesVazaoHQConjuntaTool em um único deck."""
        try:
            tool = tool_class(deck_path)
            result = tool.execute(query)
            return result
        except Exception as e:
            safe_print(f"[RESTRICOES VAZAO HQ MULTI-DECK] Erro ao executar tool em {deck_name}: {e}")
            return {
                "success": False,
                "error": str(e)
            }
