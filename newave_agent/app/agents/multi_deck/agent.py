"""
MultiDeckAgent - Classe que encapsula o graph do multi-deck agent.
"""

from typing import Optional, Generator
from langgraph.graph import StateGraph
from .state import MultiDeckState
from .graph import create_multi_deck_agent, get_multi_deck_agent as _get_agent


class MultiDeckAgent:
    """
    Agent para comparações entre múltiplos decks.
    Encapsula o LangGraph e fornece interface simplificada.
    """
    
    def __init__(self):
        """Inicializa o agent (lazy loading do graph)."""
        self._graph: Optional[StateGraph] = None
    
    @property
    def graph(self) -> StateGraph:
        """Retorna o graph compilado (lazy loading)."""
        if self._graph is None:
            self._graph = _get_agent()
        return self._graph
    
    def run_query(
        self,
        query: str,
        deck_path: str,
        session_id: Optional[str] = None
    ) -> dict:
        """
        Executa uma query de comparação no agent.
        
        Args:
            query: Query do usuário
            deck_path: Caminho do deck principal (usado para contexto)
            session_id: ID da sessão (opcional)
            
        Returns:
            Dict com resultado da comparação
        """
        from .graph import run_query as _run_query
        return _run_query(query, deck_path, session_id)
    
    def run_query_stream(
        self,
        query: str,
        deck_path: str,
        session_id: Optional[str] = None
    ) -> Generator[str, None, None]:
        """
        Executa uma query de comparação no agent com streaming.
        
        Args:
            query: Query do usuário
            deck_path: Caminho do deck principal (usado para contexto)
            session_id: ID da sessão (opcional)
            
        Yields:
            Strings no formato SSE (Server-Sent Events)
        """
        from .graph import run_query_stream as _run_query_stream
        return _run_query_stream(query, deck_path, session_id)
