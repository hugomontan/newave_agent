"""
Node que gera código Python para buscar dados do deck NEWAVE.
...
1. Use a biblioteca inewave para ler os arquivos do deck NEWAVE
...
- O deck NEWAVE está no diretório: {deck_path}
...
try:
    arquivo = NomeClasse.read(os.path.join(deck_path, "nome_arquivo.dat"))
...
Gere o código Python para responder esta pergunta usando a biblioteca inewave.
...
def coder_node(state: SingleDeckState) -> dict:
    ...
    if llm_instructions:
        ...
        # Combinar llm_instructions com relevant_docs (se houver)
        if relevant_docs:
