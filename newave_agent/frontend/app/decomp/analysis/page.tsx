...
export default function AnalysisPage() {
  ...
  const handleDeckSelection = async (decks: DeckInfo[]) => {
    ...
    try {
      const data = await loadDecompDeckFromRepo(deck.name);
  ...
  const handleUploadSuccess = async (response: UploadResponse) => {
    ...
    try {
      const sessionInfo = await getSession(response.session_id, "decomp");
    ...
    setMessages([
      {
        id: Date.now().toString(),
        role: "assistant",
        content: `Deck DECOMP carregado, faça sua consulta.`,
        timestamp: new Date(),
      },
    ]);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      for await (const event of queryDecompStream(sessionId, userMessage.content)) {
