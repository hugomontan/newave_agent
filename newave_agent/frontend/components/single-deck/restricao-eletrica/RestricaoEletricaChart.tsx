...
interface RestricaoEletricaChartProps {
  data: ChartData;
  restricao?: string;
}

export function RestricaoEletricaChart({ data, restricao }: RestricaoEletricaChartProps) {
  // Validar dados
    ...
    return (
      <div className="bg-card border border-border rounded-lg p-4 sm:p-6 w-full">
        <h3 className="text-base sm:text-lg font-semibold mb-4 text-card-foreground">
          {restricao || "Restrições Elétricas"}
        </h3>
  ...
  // Cores para as linhas (Pesado, Médio, Leve)
  const colors = ["#ef4444", "#f59e0b", "#10b981"]; // Vermelho, Laranja, Verde

  // Título dinâmico
  const chartTitle = restricao || "Limites Superiores por Patamar";
