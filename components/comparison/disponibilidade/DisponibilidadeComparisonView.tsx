...
import { DisponibilidadeComparisonTable } from "./DisponibilidadeComparisonTable";
import { DisponibilidadeComparisonChart } from "./DisponibilidadeComparisonChart";
import type { ComparisonData } from "../shared/types";

interface DisponibilidadeComparisonViewProps {
...
export function DisponibilidadeComparisonView({ comparison }: DisponibilidadeComparisonViewProps) {
  ...
  return (
    ...
    >
      ...
      {usinaInfo && (
        <div className="bg-card border border-border rounded-lg p-4 sm:p-6">
          <h3 className="text-base sm:text-lg font-semibold text-card-foreground mb-2">
            Disponibilidade Total - {usinaInfo.nome}
          </h3>
      ...
      {/* Tabela */}
      {hasTableData && (
        <DisponibilidadeComparisonTable
      ...
      {/* Gráfico */}
      {hasChartData && (
        <DisponibilidadeComparisonChart 
          data={chart_data} 
          config={chart_config}
        />
      )}
      ...
      {!hasTableData && !hasChartData && (
        <div className="bg-card border border-border rounded-lg p-4 sm:p-6">
          <p className="text-sm text-muted-foreground">
            Nenhum dado de disponibilidade disponível para comparação.
          </p>
        </div>
      )}
    </motion.div>
