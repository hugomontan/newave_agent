...
interface LimitesIntercambioComparisonTableProps {
  data: TableRow[];
}

export function LimitesIntercambioComparisonTable({ 
  data
}: LimitesIntercambioComparisonTableProps) {
    ...
    <div className="bg-card border border-border rounded-lg p-3.5 sm:p-4.5 max-w-full">
      <div className="flex items-center justify-between mb-4">
        <h4 className="text-base sm:text-lg font-semibold text-card-foreground">
          Limites de Intercâmbio por Data
        </h4>
          ...
          <tbody>
            {displayedData.map((row, index) => (
              ...
              >
                ...
                <td className="px-3.5 sm:px-4.5 py-2.5 text-sm text-card-foreground whitespace-nowrap">
                  {row.sentido ? String(row.sentido) : "-"}
                </td>
                <td className="px-3.5 sm:px-4.5 py-2.5 text-sm text-card-foreground whitespace-nowrap">
                  {formatValue((row as any).mwmed)}
                </td>
              </tr>
            ))}
          </tbody>
