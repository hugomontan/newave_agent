  ...
  try {
    ...
    if (err instanceof Error) {
      if (err.name === 'AbortError') {
        throw new Error("Timeout ao buscar decks. Verifique se o backend está rodando em http://localhost:8000");
...
export function DeckSelector({
  ...
  // Definir loadDecks ANTES do useEffect que o usa, usando useCallback para estabilizar a referência
  ...
  // Carregar decks disponíveis quando o dialog abrir
  useEffect(() => {
    if (open) {
      loadDecks().catch((err) => {
  ...
  // Fallback: se o dialog estiver aberto e não houver decks carregados, carregar
  useEffect(() => {
    if (open && decks.length === 0 && !isLoading && !error) {
      loadDecks().catch((err) => {
        console.error("Erro ao carregar decks:", err);
      });
    }
  }, [open, decks.length, isLoading, error, loadDecks]);
  
  return (
    <Dialog 
      open={open} 
      onOpenChange={(newOpen) => {
        // Se está abrindo, carregar decks imediatamente
        if (newOpen && !open) {
          setTimeout(() => {
            loadDecks().catch((err) => {
              console.error("Erro ao carregar decks:", err);
      ...
      >
        ...
        {isLoading ? (
          ...
          <div className="text-center py-8 flex-shrink-0">
            <p className="text-muted-foreground">Nenhum deck disponível</p>
