...
export async function fetchAvailableDecks(): Promise<DeckInfo[]> {
  const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000"}/decks/list`);
  if (!response.ok) {
    throw new Error("Erro ao buscar decks disponíveis");
  }
