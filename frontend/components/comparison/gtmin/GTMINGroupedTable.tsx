...
export function GTMINGroupedTable({ differences, deck1Name, deck2Name, firstColumnLabel = "Usina" }: GTMINGroupedTableProps) {
          ...
          <table className="min-w-full border-collapse table-fixed">
            <thead>
              <tr className="border-b border-border bg-background/50">
                <th className="w-1/4 px-3 sm:px-4 py-3 text-left text-xs font-semibold text-card-foreground uppercase tracking-wider">
                  {firstColumnLabel}
                </th>
                {showPeriodoColumn && (
                  <th className="w-1/4 px-3 sm:px-4 py-3 text-left text-xs font-semibold text-card-foreground uppercase tracking-wider">
                ...
                <th className="w-1/4 px-3 sm:px-4 py-3 text-right text-xs font-semibold text-blue-400 uppercase tracking-wider">
                  {deck1Name}
                </th>
                <th className="w-1/4 px-3 sm:px-4 py-3 text-right text-xs font-semibold text-purple-400 uppercase tracking-wider">
                  {deck2Name}
                </th>
              </tr>
            </thead>
            <tbody>
              {displayedData.map((diff, index) => (
                ...
                >
                  ...
                  {showPeriodoColumn && (
                    <td className="px-3 sm:px-4 py-2.5 text-sm text-muted-foreground truncate" title={diff.periodo_coluna || "-"}>
                      {diff.periodo_coluna || "-"}
                    </td>
                  )}
