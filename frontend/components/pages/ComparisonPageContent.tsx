...
export function ComparisonPageContent({ model }: ComparisonPageContentProps) {
  ...
  const initializeComparisonWithDecks = async (decks: DeckInfo[]) => {
    ...
    try {
      ...
      const data = await config.initComparisonFn(deckNames);
  ...
  useEffect(() => {
    ...
    const initializeDefault = async () => {
      try {
        setIsLoading(true);
        const data = await config.initComparisonFn();
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      for await (const event of config.streamFn(sessionId, userMessage.content, "comparison")) {
  ...
  return (
    <main className="flex flex-col h-screen bg-background overflow-hidden">
      ...
      <DeckSelector
        ...
        onSelect={initializeComparisonWithDecks}
        model={model}
