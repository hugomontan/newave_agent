...
const MODEL_CONFIG = {
  newave: {
    ...
    deleteSessionFn: (id: string) => deleteSession(id),
    deckLabel: "NEWAVE",
  },
  decomp: {
    ...
    deleteSessionFn: (id: string) => deleteSession(id, "decomp"),
    deckLabel: "DECOMP",
  },
};
...
export function AnalysisPageContent({ model }: AnalysisPageContentProps) {
  ...
  const handleDeckSelection = async (decks: DeckInfo[]) => {
    ...
    try {
      const data = await config.loadDeckFn(deck.name);
  ...
  const handleUploadSuccess = async (response: UploadResponse) => {
    ...
    try {
      const sessionInfo = await config.getSessionFn(response.session_id);
    ...
    setMessages([
      {
        id: Date.now().toString(),
        role: "assistant",
        content: `Deck ${config.deckLabel} carregado, faça sua consulta.`,
        timestamp: new Date(),
      },
    ]);
  ...
  const onClearSession = async () => {
    if (sessionId) {
      try {
        await config.deleteSessionFn(sessionId);
      } catch (err) {
  ...
  const handleReindex = async () => {
    ...
    try {
      const result = await config.reindexFn();
      setMessages((prev) => [
        ...prev,
        {
          id: Date.now().toString(),
    ...
    <main className="flex flex-col h-screen bg-background overflow-hidden">
        ...
        <div className="border-t border-border bg-background">
          <div className="max-w-3xl mx-auto px-4 py-4">
            ...
            <p className="text-xs text-center text-muted-foreground mt-2">
              {config.deckLabel} Agent pode cometer erros. Verifique informações importantes.
            </p>
          </div>
        </div>
      </div>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="bg-card border-border">
          <DialogHeader>
            <DialogTitle className="text-card-foreground">Upload do Deck {config.deckLabel}</DialogTitle>
          </DialogHeader>
          <FileUpload model={model} onUploadSuccess={handleUploadSuccess} />
        </DialogContent>
      </Dialog>
